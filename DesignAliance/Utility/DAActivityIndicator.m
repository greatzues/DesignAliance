...
@implementation DAActivityIndicator
...
- (void)setMode:(DAActivityMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    //，
    if (mode == DAActivityModeDeterminate) {
        if ([NSThread isMainThread]) {
            [self updateProgress];
            [self setNeedsDisplay];
        } else {
...
- (void)updateIndicators {
    ...
    else if (mode == DAActivityModeCustomView && self.customView != nil){
...
+ (DAActivityIndicator *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    DAActivityIndicator *hud = [[DAActivityIndicator alloc] initWithView:view];
    [view addSubview:hud];
    [hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    ...
    for (UIView *v in [view subviews]) {
        if ([v isKindOfClass:[DAActivityIndicator class]]) {
            viewToRemove = v;
        }
    }
    if (viewToRemove != nil) {
        DAActivityIndicator *HUD = (DAActivityIndicator *)viewToRemove;
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.animationType = DAActivityAnimationFade;
        self.mode = DAActivityModeIndeterminate;
        self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == DAActivityAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == DAActivityAnimationZoom) {
